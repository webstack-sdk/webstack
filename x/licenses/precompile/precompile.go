...
// Precompile is the EVM precompiled contract that exposes the x/licenses module.
...
// returned contract is intended to be registered at licensestypes.PrecompileAddress
...
func NewPrecompile(
	...
	return &Precompile{
		...
		ABI:         ABI,
		keeper:      keeper,
		msgServer:   licenseskeeper.NewMsgServerImpl(keeper),
		queryServer: licenseskeeper.NewQuerier(keeper),
		addrCdc:     addrCdc,
	}
}
...
func (p Precompile) Logger(ctx sdk.Context) log.Logger {
	return ctx.Logger().With("evm extension", licensestypes.ModuleName)
}
